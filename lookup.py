...
def normalize_attack_id(raw: str):
    if not raw:
        return None
    m = ATTACK_ID_RE.search(raw)
    return m.group(0).upper() if m else None
    
def get_attack_id(tech, query):
...
def get_sigma_root(sigma):
    ...
    if not sigma_root.exists():
        console.print(f"[red]Sigma folder not found at {sigma}[/red]")
        return
...
def get_mitigations_for_technique(mitre, tech):
    try:
        tech_stix_id = tech["id"]
